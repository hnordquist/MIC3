...
CListCtrlStyled::~CListCtrlStyled()
{ 	
//	TRACE("CListCtrlStyled::DTR\n");
	// Must free if it's not already done
	//
...
/*
		...
		for (int nSubItem = 0; nSubItem < 4; nSubItem++)
		{
			...
			{	// Get lParam value for this item
				//
				char szText[1024];
				GetItemText( nItem, nSubItem, szText, sizeof(szText));
				if(strcmp(szText,pFindInfo->psz)==0) 
					return nItem;
			}
		}
*/
...
COLORREF CListCtrlStyled::GetTxtColor(int nItem, int nSubItem)
{
//		bool set = false;
	LVITEM pItem;
	InitLVITEM(nItem,0,&pItem);
